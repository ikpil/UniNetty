...
namespace UniNetty.Buffers
{
    ...
    static class UnsafeByteBufferUtil2
    {
        const byte Zero = 0;
        ...
        internal static Task<int> SetBytesAsync(AbstractByteBuffer buf, ArraySegment<byte> addr, int index, Stream input, int length, CancellationToken cancellationToken)
        {
            if (length == 0)
            {
                return TaskEx.Zero;
            ...
            return tmpBuf.SetBytesAsync(0, input, length, cancellationToken)
                .ContinueWith(t =>
                {
                    try
                    {
                        ...
                        if (read > 0)
                        {
                            PlatformDependent.CopyMemory(tmpBuf.Array, tmpBuf.ArrayOffset, addr.AsSpan(index), read);
                        }
